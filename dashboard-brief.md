...
org                 str | null   organisation name from ORG.md
workspace           str | null   path to this opportunity's folder
direction           [str]        the angle, verbatim from the director agent
draft               object | {}  present only when the gate fired
  question                  str
  answer                    str
  claims                    [ { text: str, grounded: bool, sources: [str] } ]
  unevidenced_gaps          [str]
timeline            object | {}
  deadline                  str
  total_hours_estimate      float
  steps                     [ { days_before_deadline: int, task: str, owner_role: str } ]
files_written       [ { name: str, size: str } ]   real files on disk
outcome             { status: "pending"|"awarded"|"rejected", note: str }

revisit             [str]  full sentences, e.g.
                           "Southwark Council — four awards, £22,500 in total,
                            last said yes August 2023"
```

Everything from `org` down is present only when the gate fired. On a silent run
`draft`, `timeline`, `files_written` and `workspace` are empty or null, and those
panels should not render at all rather than rendering empty.
